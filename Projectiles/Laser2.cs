...
namespace OnePunch.Projectiles
{
	...
	public class Laser2: ModProjectile
	{
		...
		private const float MaxChargeValue = 250f;
		...
		public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
		{
			...
			for (float i = transDist; i <= Distance; i += step)
			{
				...
				spriteBatch.Draw(texture, origin - Main.screenPosition,
					new Rectangle(0, 260, 280, 260), i < transDist ? Color.Transparent : c, r,
					new Vector2(280 * .5f, 260 * .5f), scale, 0, 0);
			}
			#endregion

			#region Draw laser tail
			spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
				new Rectangle(0, 0, 280, 260), Color.White, r, new Vector2(280 * .5f, 260 * .5f), scale, 0, 0);
			#endregion

			#region Draw laser head
			spriteBatch.Draw(texture, start + (Distance + step) * unit - Main.screenPosition,
				new Rectangle(0, 520, 280, 260), Color.White, r, new Vector2(280 * .5f, 260 * .5f), scale, 0, 0);
			#endregion
		}
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
		{
			...
			if (AtMaxCharge)
			{
				...
				return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), player.Center,
					player.Center + unit * Distance, 220, ref point);
			}
			return false;
		}
